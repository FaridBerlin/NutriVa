...
function calculateTDEE(bmr, activityLevel) {
  ...
  if (!factor) {
    throw new Error(`Activity level must be one of: ${Object.keys(activityFactors).join(', ')}`);
  }
...
function adjustCaloriesForGoal(tdee, goal) {
  ...
  if (adjustment === undefined) {
    throw new Error(`Goal must be one of: ${Object.keys(adjustments).join(', ')}`);
  }
...
function calculateMacroTargets(dailyCalories, goal) {
  ...
  switch (goal.toLowerCase()) {
    ...
    case 'aggressive_loss':
      ...
      macroSplit = { protein: 0.40, carbs: 0.40, fat: 0.20 };
      break;
    case 'weightGain':
    case 'aggressive_gain':
      // Higher carbs for energy and muscle building
      macroSplit = { protein: 0.30, carbs: 0.50, fat: 0.20 };
      break;
    case 'maintenance':
    default:
      // Balanced split
...
function selectMealForCategory(params) {
  ...
  try {
    ...
    if (!selected || selected.length === 0) {
      throw new Error(`Could not select meal for ${category} - all options exhausted`);
    }
 ...
 */
function validateNutrition(actual, target, tolerance = 0.10) {
  const warnings = [];
  ...
  } else if (actual.calories > target.calories + tolerance_amount(target.calories)) {
    warnings.push(
  ...
  } else if (actual.protein > target.protein + tolerance_amount(target.protein)) {
    warnings.push(
...
function generatePlanNutritionSummary(planData, targets) {
  ...
  return {
    ...
    validationSummary: {
      averageCalorieVariance: Math.round(averages.calories - targets.dailyCalories),
...
async function generateMealPlan(params) {
  ...
  const calorieDistribution = {
    ...
    snack: dailyCalories * 0.10  // Only if mealsPerDay > 3
  };
  ...
  const mealPlan = {
    ...
    nutritionTargets: {
      ...
      userMetrics: { weight, height, age, gender, activityLevel, goal, bmr, tdee }
    },
  ...
  for (let day = 1; day <= duration; day++) {
    ...
    try {
      ...
      throw new Error(`Failed to generate meals for day ${day}: ${error.message}`);
    }
  }
  ...
  if (mealPlan.nutritionSummary.averageCalorieVariance < -200) {
    mealPlan.warnings.push('Plan is significantly lower in calories than target');
  }
  if (mealPlan.nutritionSummary.averageCalorieVariance > 200) {
    mealPlan.warnings.push('Plan is significantly higher in calories than target');
  }
