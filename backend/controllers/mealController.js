...
export const updateMeal = async (req, res, next) => {
    ...
    if (meal.createdBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "You are not allowed to update this meal" });
    }
...
export const getMealsByType = async (req, res, next) => {
    ...
    //validate type
    const allowedTypes = ["balanced", "high-protein", "low-carb", "keto", "vegan", "veg", "gluten-free"];
...
export const deleteMeal = async (req, res, next) => {
    ...
    // Check meal owner
    if (meal.createdBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "You are not allowed to delete this meal" });
    }
