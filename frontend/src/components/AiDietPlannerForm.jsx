...
export default function AiDietPlannerForm({ onPlanGenerated, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Say which engine actually produced the plan. The AI path silently
        // falls back to templates when the model call fails, and without this
        // there is no way to tell the two apart.
        const messages = {
          ai: 'Meal plan generated by AI, tailored to your profile.',
          'ai-partial-templates': `AI generated ${result.generationDetail?.aiDays ?? 0} of ${
            (result.generationDetail?.aiDays ?? 0) +
            (result.generationDetail?.templateDays ?? 0)
          } days; the rest came from templates.`,
          'templates-ai-rejected':
            'AI results did not meet your calorie targets, so templates were used.',
          'templates-ai-failed':
            'AI was unavailable, so your plan was built from templates.',
          templates: 'Meal plan built instantly from templates.',
        }
        setSuccess(
          messages[result.generationMethod] ||
