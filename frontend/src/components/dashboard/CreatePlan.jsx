import { Sparkles, CheckCircle, UserCircle, ArrowRight, Wand2 } from 'lucide-react';
...
export default function CreatePlan({ profile, onGenerate }) {
  ...
  const profileInfo = [
    ...
    { label: 'Daily Calories', value: profile?.dailyCalories ? `${profile.dailyCalories} kcal` : '2000 kcal' },
    { label: 'Current BMI', value: profile?.bmi || '24.5' },
    { label: 'Activity Level', value: profile?.activityLevel || 'Moderate' },
    { label: 'Duration', value: profile?.duration ? `${profile.duration} Days` : '30 Days' }
  ];
            ...
            {profileInfo.map((item, index) => (
              <div 
                ...
                className={`flex justify-between py-2 ${
                  index < profileInfo.length - 1 ? 'border-b border-gray-100' : ''
                }`}
              >
