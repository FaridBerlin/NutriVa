...
export default function DashboardStats({ stats }) {
  ...
  const cardBg = isDark
    ? 'bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border-slate-700/50'
  ...
  const hoverShadow = isDark ? 'hover:shadow-amber-500/10' : 'hover:shadow-lg'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative pb-4">
        <h2
          className={`text-3xl font-bold ${textPrimary} tracking-tight`}
          style={{ fontFamily: "'Space Grotesk', sans-serif" }}
        >
          Dashboard Overview
        </h2>
        <div
          className={`absolute bottom-0 left-0 h-1 w-32 rounded-full ${
            isDark
