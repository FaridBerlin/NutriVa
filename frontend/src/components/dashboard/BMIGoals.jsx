...
export default function BMIGoals({ bmi, bmiCategory, goals }) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-textDark mb-6">BMI & Goals Progress</h2>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* BMI Analysis with Gauge */}
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
          <h3 className="text-xl font-semibold text-center mb-4">BMI Analysis</h3>
          <div className="flex flex-col items-center">
            <div className="relative w-48 h-48">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-4xl font-bold" style={{ color: getBMIColor(bmi) }}>
                  {bmi?.toFixed(1) || '24.5'}
                </span>
                <span className="text-sm text-textLight">BMI</span>
              </div>
            </div>
            <p className="text-lg font-semibold mt-2" style={{ color: getBMIColor(bmi) }}>
              {bmiCategory || 'Healthy'}
            </p>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
          <h3 className="text-xl font-semibold text-center mb-4">Macros Distribution</h3>
          ...
          <div className="flex justify-center gap-4 mt-2">
            {macrosData.map((item) => (
              <div key={item.name} className="flex items-center gap-2">
                ...
                <span className="text-sm text-textLight">{item.name}: {item.value}g</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100 mt-6">
        <h3 className="text-xl font-semibold text-center mb-4">Daily Goals Progress</h3>
