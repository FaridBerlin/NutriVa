...
export function ProfileProvider({ children }) {
  ...
  const createProfile = async (profileData) => {
    ...
    try {
      const { data, error: apiError } = await profileApi.createProfile(profileData);
  ...
  const updateProfile = async (profileData) => {
    ...
    try {
      const { data, error: apiError } = await profileApi.updateProfile(profileData);
  ...
  // Computed values
  const isProfileComplete = Boolean(profile && profile.age && profile.gender && profile.height && profile.weight);
  
  const calorieGoal = nutritionTargets?.targetCalories || 0;
  
  const profileProgress = profile ? (() => {
    const requiredFields = ['age', 'gender', 'height', 'weight', 'activityLevel', 'dietaryGoal'];
    const filledFields = requiredFields.filter(field => profile[field] !== null && profile[field] !== undefined);
